...
ShadowPipeline::ShadowPipeline(const VkCtx& ctx, VkRenderPass renderPass) :_ctx(ctx) {
    ...
    check_vk_result(vkCreateDescriptorSetLayout(ctx.device(), &layoutInfo, allocCallback, &_descriptorLayout));
...
VkPipeline ShadowPipeline::pipeline() const
{
    return _pipeline;
}
...
VkDescriptorSetLayout ShadowPipeline::descriptorSetLayout() const
{
